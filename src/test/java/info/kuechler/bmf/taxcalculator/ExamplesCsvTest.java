...
public class ExamplesCsvTest {
    ...
    private static final WriterConfigurer CONF_SPECIAL = new WriterConfigurer() {
        ...
        @Override
        public void configure(Writer writer, final int taxClass) throws ReadWriteException {
            ...
            // unknown, not documented for test data, but "1" returns the correct result
            writer.set("KRV", 1);
        }
    };

    private static final WriterConfigurer CONF_SPECIAL_PKV = new WriterConfigurer() {

        @Override
        public void configure(Writer writer, final int taxClass) throws ReadWriteException {
            // 1 = ausschließlich privat krankenversicherte Arbeitnehmer OHNE Arbeitgeberzuschuss
            writer.set("PKV", 1);
            // Merker für die Vorsorgepauschale 2 = der Arbeitnehmer ist NICHT in der gesetzlichen
    ...
    private static final WriterConfigurer CONF_GENERAL = new WriterConfigurer() {
    ...
    private static final WriterConfigurer CONF_GENERAL_PKV = new WriterConfigurer() {

        @Override
        public void configure(Writer writer, final int taxClass) throws ReadWriteException {
            // Krankenversicherung: 0 = gesetzlich krankenversicherte Arbeitnehmer
            writer.set("PKV", 0);
            // pensions fund: west germany
            writer.set("KRV", 0);
            // 1, wenn er der Arbeitnehmer den Zuschlag zur sozialen Pflegeversicherung zu zahlen hat, sonst 0.
    ...
    private static final WriterConfigurer CONF_GENERAL_PKV_09 = new WriterConfigurer() {
    ...
    private static final WriterConfigurer CONF_GENERAL_PKV_11 = new WriterConfigurer() {
    ...
    @Parameter(value = 3)
    public WriterConfigurer configurer;
            ...
            for (final CSVRecord record : parser) {
                if (parser.getCurrentLineNumber() == 1L) {
                    ...
                    for (int i = 1; i < record.size(); i++) {
                        ...
                        final Writer writer = factory.create(factory.getYearKey(month, year));
                        writer.setAllToZero();
                        // 1. yearly payment
                        // 2. tax class
                        // 3. income in cent
